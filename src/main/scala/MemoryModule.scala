...
class MemoryModule extends Module {
    ...
    val io = IO(new Bundle {
        ...
        val Inport1En = Bool();
        val MemWrite = Bool();
        val SwitchData = Vec(switches, UInt(width.W))
        val WrData = UInt(width.W);

        val RdData = UInt(width.W);
    })

    val WrEn = io.MemWrite & (io.addr >= 0.U) & (io.addr < maxAddr)
    ...
    io.RdData := MuxCase(0.U, Array(
        (mux_sel == 1.U).B -> Inport1.io.reg,
        (mux_sel == 2.U).B -> Inport0.io.reg,
